...
public sealed class SkillsOptions
{
    ...
    /// Скрипты выполняет наш собственный раннер — см. ProcessSkillScriptRunner.
    ///
    /// Касается только скиллов из <see cref="Directories"/> — тех, что положил на сервер
    /// администратор. Загруженные пользователями идут отдельным флагом, см.
    /// <see cref="AllowUserScripts"/>.
    /// </summary>
    public bool AllowScripts { get; init; }

    /// <summary>
    /// Разрешить запуск скриптов в скиллах, загруженных пользователями.
    ///
    /// Выключено, и включать это по умолчанию нельзя. Скрипт из папки на сервере кладёт
    /// туда администратор — модель лишь выбирает, какой из них запустить. Скилл же
    /// загружает пользователь через API, и разрешить его скриптам выполняться означает
    /// дать любому пользователю запускать произвольный код на сервере приложения.
    /// Включать имеет смысл только вместе с песочницей и модерацией загружаемых скиллов.
    /// </summary>
    public bool AllowUserScripts { get; init; }

    /// <summary>
    /// Папка, в которую разворачиваются архивы скиллов из файлового хранилища. Пусто —
    /// подпапка во временном каталоге системы.
    ///
    /// Содержимое папки — кэш и ничего больше: его можно удалить целиком в любой момент,
    /// скиллы развернутся заново из хранилища.
    /// </summary>
    public string? CacheDirectory { get; init; }

    /// <summary>
    /// Сколько часов держать в кэше развёрнутый скилл, к которому не обращались.
    /// Минимум — час.
    /// </summary>
    public int CacheLifetimeHours { get; init; } = 24;
