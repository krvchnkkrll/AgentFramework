...
/// <summary>
/// Скиллы — markdown-инструкции для отдельных видов задач. В системный промпт уезжает только
/// список «имя — описание» из базы, а текст модель подтягивает инструментом load_skill, когда
/// решит, что скилл ей нужен: тогда он и качается из файлового сервиса. Это позволяет держать
/// десятки инструкций, не раздувая каждый запрос.
///
/// Скрипты скиллы не запускают — скилл это только текст.
/// </summary>
